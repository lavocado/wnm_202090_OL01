...
function recommendedSimilar($cat,$id=0,$limit=6) {
	$result = makeQuery(makeConn(),"SELECT * FROM `products` WHERE `id`<>$id ORDER BY rand() DESC LIMIT $limit");
	echo "<div class='grid gap'>", array_reduce($result, 'productListTemplate2'),"</div>";
}
